...
const MAX_ID_ENV = Number(process.env.FIRECRAWL_MAX_ID || 0); // manual anchor if auto-detect finds none
...
async function onRoundComplete(job) {
  if (job.round === 1) {
    ...
    // Newest first (higher article id).
    ...
    const have = new Set(round2.map(keyOf));

    // Diagnostics: how many article links we harvested, and the newest id seen.
    const anchorId = Math.max(MAX_ID_ENV, 0, ...candidates.map((u) => extractId(u) || 0));
    job.harvested = candidates.length;
    job.maxId = anchorId;

    // ID back-fill: the site's listings may hide article links behind post-backs,
    // so also probe news article ids downward from the newest one we found.
    // Gaps/deleted ids just come back thin and get skipped during ingest.
    if (ID_BACKFILL && anchorId > 0) {
      for (let id = anchorId; id > 0 && round2.length < room; id--) {
        const u = newsArticleUrl(id);
        const k = keyOf(u);
