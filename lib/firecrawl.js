...
const SELK_BASE = (() => {
  try {
    return new URL(CRAWL_URL).origin + '/selk/';
  } catch {
    return 'https://www.icpac.org.cy/selk/';
  }
})();

export function newsArticleUrl(id) {
  return `${SELK_BASE}newdetails.aspx?id=${id}&catid=1001`;
}

// Article links found on a scraped page. Scans BOTH the links array and the
// markdown text for the newdetails/eventdetails?id=N pattern (so it works even
// when Firecrawl returns the links relative or omits them), and rebuilds a
// canonical absolute URL for each.
export function articleLinksFrom(item) {
  if (!item) return [];
  const hay = (item.markdown || '') + '\n' + JSON.stringify(item.links || []);
  const out = new Map();
  // Article detail pages are newdetails.aspx (news) and eventdetails.aspx (events).
  for (const m of hay.matchAll(/\b(new|event)details\.aspx\?id=(\d+)/gi)) {
    const type = m[1].toLowerCase();
    const catid = type === 'new' ? '1001' : '1002';
    const url = `${SELK_BASE}${type}details.aspx?id=${m[2]}&catid=${catid}`;
